...
class GaussianBlurTransform(object):
    def __init__ (self, kernel_size):
        ...
        self.transform = GaussianBlur(kernel_size = self.kernel_size)

    def __call__(self, image, mask, joints, area):
        rgb_image = torch.from_numpy(image[:,:,:3]).permute(2,0,1)
        image[:,:,:3] = self.transform(rgb_image).numpy().transpose(1,2,0)
        return image, mask, joints, area


class MotionBlurTransform(object):
    def __init__(self, kernel_size, num_shift, shift_range):
        self.kernel_size = kernel_size
