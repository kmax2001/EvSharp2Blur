...
def transform_preds(coords, center, scale, output_size):
    ...
    batch_mode = coords.ndim ==3
    if not batch_mode:
        coords = coords[None]
    N, K, _ = coords.shape
    trans = get_affine_transform(center, scale, 0, output_size, inv=1)
    coords_homo = np.concatenate([coords[:,:,:2].cpu().numpy(), np.ones((N, K, 1))], axis = -1)
    transformed_coords = np.einsum('bnd,dc->bnc', coords_homo, trans.T)
...
def resize_align_multi_scale_batch(image, input_size, current_scale, min_scale):
    result = []
    B, H, W, C= image.shape
    ...
    image_resized = torch.Tensor([cv2.warpAffine(image[i].cpu().numpy(), trans, size_resized) for i in range(B)]).to(image.device)
...
def get_final_preds(grouped_joints, center, scale, heatmap_size):
    ...
    for person in grouped_joints:
        try:
            keypoint_score = person[:,:,2:3]
        except:
            pdb.set_trace()
        n, k , _ = person.shape
        joints = np.zeros((n,k,2))
        joints = transform_preds(person, center, scale, heatmap_size)
        joints = np.concatenate([joints,keypoint_score], axis = 2)
