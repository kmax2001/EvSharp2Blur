...
def get_heat_value_batch(pose_coord, heatmap):
    B, _, h, w = heatmap.shape
    heatmap_nocenter = heatmap[:, :-1].flatten(2,3).transpose(1,2)

    y_b = torch.clamp(torch.floor(pose_coord[:,:,:,1]), 0, h-1).long()
    x_l = torch.clamp(torch.floor(pose_coord[:,:,:,0]), 0, w-1).long()
    index = (y_b * w + x_l).unsqueeze(-1)  

    heatval = torch.gather(heatmap_nocenter, 1, index.squeeze(-1))  
    return heatval
...
def nms_core_batch(cfg, pose_coord, heat_score, selection_ind_mask):
...
def pose_nms_batch(cfg, heatmap_avg, poses, select_ind_mask):
    """
    NMS for the regressed poses results.
    ...
    heatval = get_heat_value_batch(pose_coord, heatmap_avg)  
    ...
    poses = poses.reshape(B,num_people, num_joints,-1)
    keep_pose_inds = nms_core_batch(cfg, pose_coord, heat_score, select_ind_mask)
    heat_score = heat_score.reshape(B, -1)
    poses_all = []
    heat_scores_all =[]
    idx_from = []
