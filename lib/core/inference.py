...
def get_reg_poses(offset, num_joints):
    ...
    return poses
def get_reg_poses_batch(offset, num_joints):
...
def offset_to_pose_batch(offset, flip=True, flip_index=None):
    B, num_offset, h, w = offset.shape
    num_joints = int(num_offset/2)
    reg_poses = get_reg_poses_batch(offset, num_joints) 
...
def get_multi_stage_outputs(cfg, model, image, event, with_flip=False, modality = None):
    ...
    if with_flip:
        ...
        event = torch.flip(event, [3])
        event[:, :, :, :-3] = event[:, :, :, 3:]
        
        
        heatmap_flip, offset_flip = model(torch.cat([image, event], dim=1))
...
def get_multi_stage_outputs_batch(cfg, model, image, event, with_flip=False, modality = None):
    ...
    if 'image' in modality:
        ...
        inp = event
    heatmap, offset =  model(inp)
    posemap = offset_to_pose_batch(offset, flip=False)
    if with_flip:
        if 'coco' in cfg.DATASET.DATASET:
            flip_index_heat = FLIP_CONFIG['COCO_WITH_CENTER']
            flip_index_offset = FLIP_CONFIG['COCO']
...
def get_multi_stage_outputs_video(cfg, model, image, event,voxel_10, voxel_01, with_flip=False, modality = None):
    if modality is None:
        modality = cfg.MODEL.MODALITY
    if 'image' in modality:
        inp = image
        ...
        inp = event
    data = {"image": image.unsqueeze(0), "event":event.unsqueeze(0), "voxel_10": voxel_10.unsqueeze(0), "voxel_01": voxel_01.unsqueeze(0)}
    heatmap, offset, _,_,_,_ =  model(inp.unsqueeze(0), data)
    posemap = offset_to_pose_batch(offset, flip=False)
    if with_flip:
        if 'coco' in cfg.DATASET.DATASET:
            flip_index_heat = FLIP_CONFIG['COCO_WITH_CENTER']
...
def hierarchical_pool_batch(cfg, heatmap):
    pool1 = torch.nn.MaxPool2d(3, 1, 1)
    pool2 = torch.nn.MaxPool2d(5, 1, 2)
    pool3 = torch.nn.MaxPool2d(7, 1, 3)
    map_size = (heatmap.shape[1]+heatmap.shape[2])/2.0
...
def get_maximum_from_heatmap_batch(cfg, heatmap):
    B = heatmap.shape[0]
    maxm = hierarchical_pool_batch(cfg, heatmap)
    maxm = torch.eq(maxm, heatmap).float()
    heatmap = heatmap * maxm
...
def aggregate_results_batch(
    ...
    pose_ind, ctr_score, select_ind_mask= get_maximum_from_heatmap_batch(cfg, center_heatmap) # (batch, 후보 수), (batch, 후보 수), (batch, 후보 수)-> mask 임 
