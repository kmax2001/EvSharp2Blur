...
def do_train(cfg, model, data_loader, loss_factory, optimizer, epoch,
             output_dir, tb_log_dir, writer_dict, ev_teacher = None, im_ev_teacher = None):
    ...
    for i, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            inp_heatmap,inp_offset = ev_teacher(event)
            heatmap2, offset2 = im_ev_teacher(torch.cat([image, event], dim = 1))
            inp_heatmap = torch.cat([inp_heatmap, heatmap2], dim =1)
            inp_offset = torch.cat([inp_offset, offset2], dim = 1)
        pheatmap, poffset = model(inp_heatmap, inp_offset, event, flow, image)
