...
def get_multi_stage_outputs(cfg, model, image, event, flow, with_flip=False, modality = None, teacher1= None, teacher2= None):

    with torch.no_grad():
        heatmap1, offset1 = teacher1(event)
        heatmap2, offset2 = teacher2(torch.cat([image, event], dim = 1))
    heatmap, offset = model(torch.cat([heatmap1, heatmap2], dim = 1), torch.cat([offset1, offset2], dim = 1), event, flow, image)
