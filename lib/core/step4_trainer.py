...
from core.inference_batch import get_multi_stage_outputs
    ...
    for i, (data) in enumerate(data_loader):
        ...
        if True:
            with torch.no_grad():
                ...
                event_heatmap, event_offset = teacher_model3(event_resized)
                blur_heatmap, blur_offset = teacher_model2(torch.cat([image_resized, event_resized], dim = 1))
                tmp_heatmap, tmp_offset = teacher_model1(torch.cat([event_heatmap, blur_heatmap], dim = 1), torch.cat([event_offset, blur_offset], dim = 1), event_resized, t_flow_resized, image_resized)
                ...
                heatmap_sum, poses, select_ind_mask, select_ind_mask_crossed = aggregate_results_crosscheck(cfg, heatmap_sum, poses, tmp_heatmap, posemap, 1, do_print= (i%100 ==0) and (epoch%2==0))
                ...
                for idx, pose in enumerate(poses):
                    if pose.shape[0]:
                        if sum(idx_from[idx]):
                            pose1 = match_pose_to_heatmap(cfg, pose[:,:,:3][idx_from[idx].squeeze(-1)], heatmap_sum[2*idx+1:2*idx+2])[0]
                            score1 = scores[idx][idx_from[idx].squeeze(-1)]
                            if sum(~idx_from[idx]):
                                pose2 = match_pose_to_heatmap(cfg, pose[:,:,:3][~idx_from[idx].squeeze(-1)], heatmap_sum[2*idx:2*idx+1])[0]
                                score2 = scores[idx][~idx_from[idx].squeeze(-1)]
                                pose_res = torch.cat([pose1,pose2], dim = 0)
                            ...
                            pose_res = match_pose_to_heatmap(cfg, pose[:,:,:3][~idx_from[idx].squeeze(-1)], heatmap_sum[2*idx:2*idx+1])[0]
                    else:
                        pose_res = pose
                    final_poses.append(pose_res)

                final_poses = get_final_preds(
                    final_poses, center, scale_resized, base_size
                )
        ...
        s_pheatmap, s_poffset = model(torch.cat([image, event], dim = 1))
