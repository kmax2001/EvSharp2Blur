...
def main():
    ...
    model = eval('models.'+cfg.MODEL.NAME+'.get_pose_net')(
        cfg, is_train=False, modality= cfg.DATASET.DOMAIN_MODALITY.SOURCE+cfg.DATASET.DOMAIN_MODALITY.TARGET
    )
    torch.cuda.set_device(0)
    if cfg.TEST.MODEL_FILE:
        logger.info('=> loading model from {}'.format(cfg.TEST.MODEL_FILE))

        model.load_state_dict(torch.load(cfg.TEST.MODEL_FILE, map_location= "cuda")['student_state_dict'], strict=True)
    else:
    ...
    for i, (images, events, image_info, _, gt_joints) in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            for scale in sorted(cfg.TEST.SCALE_FACTOR, reverse=True):
                ...
                heatmap, posemap = get_multi_stage_outputs(
                    cfg, model, image_resized, event_resized, cfg.TEST.FLIP_TEST
                )
        ...
        if True:
            ...
            save_valid_image(image, gt_joints.squeeze(0), final_poses, scores, '{}.jpg'.format(prefix),color_map = "white", dataset='CROWDPOSE')
